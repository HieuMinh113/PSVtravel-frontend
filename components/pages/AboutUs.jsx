...
function MomentCard({ m, i, onOpen }) {
  return (
    ...
    >
      <Image src={m.src} alt={m.caption} fill sizes="(max-width: 640px) 50vw, 25vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
    ...
    <div>
      ...
      <section className="bg-foam py-16 sm:py-20">
        <div className="mx-auto grid max-w-7xl grid-cols-1 items-center gap-10 px-5 sm:px-8 lg:grid-cols-2 lg:gap-16">
          <SectionReveal>
            <span className="text-xs font-bold uppercase tracking-[0.25em] text-teal-500">Câu chuyện của chúng tôi</span>
          ...
          <SectionReveal delay={0.1} className="relative">
            <div className="grid grid-cols-2 gap-4">
              <img src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?q=80&w=700&auto=format&fit=crop" alt="Văn phòng PSVTravel" className="col-span-2 h-56 w-full rounded-2xl object-cover shadow-md" />
              <img src="https://images.unsplash.com/photo-1600880292203-757bb62b4baf?q=80&w=500&auto=format&fit=crop" alt="Đội ngũ PSVTravel" className="h-40 w-full rounded-2xl object-cover shadow-md" />
              <img src="https://images.unsplash.com/photo-1543269865-cbf427effbad?q=80&w=500&auto=format&fit=crop" alt="Họp nhóm PSVTravel" className="h-40 w-full rounded-2xl object-cover shadow-md" />
            </div>
            <div className="pointer-events-none absolute -bottom-6 -left-6 h-24 w-24 rounded-full bg-teal-400/20 blur-2xl" />
            <div className="pointer-events-none absolute -right-6 -top-6 h-24 w-24 rounded-full bg-ocean-400/20 blur-2xl" />
          </SectionReveal>
        </div>
      </section>
      ...
      <section className="relative overflow-hidden bg-deep-gradient py-16 text-white sm:py-20">
        <div className="mx-auto max-w-7xl px-5 sm:px-8">
          <SectionReveal className="mx-auto max-w-2xl text-center">
            <span className="text-xs font-bold uppercase tracking-[0.25em] text-teal-300">Giá trị cốt lõi</span>
            <h2 className="mt-3 font-display text-3xl font-bold sm:text-4xl">Điều làm nên PSVTravel</h2>
          </SectionReveal>
          <div className="mt-14 grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            {values.map((v, i) => (
              <SectionReveal key={v.title} delay={i * 0.1} className="text-center">
      ...
      <section className="bg-foam py-16 sm:py-20">
        <div className="mx-auto max-w-5xl px-5 sm:px-8">
          <SectionReveal className="text-center">
            <span className="text-xs font-bold uppercase tracking-[0.25em] text-teal-500">Cột mốc</span>
            <h2 className="mt-3 font-display text-3xl font-bold text-deep-900 sm:text-4xl">Hành trình phát triển</h2>
          </SectionReveal>

          <div className="relative mt-14">
            <div className="absolute left-[15px] top-0 h-full w-px bg-ocean-100 sm:left-1/2" />
            <div className="space-y-10">
              {milestones.map((m, i) => (
                ...
                >
                  ...
                  <span className="absolute left-[15px] top-1 grid h-8 w-8 -translate-x-1/2 place-items-center rounded-full bg-ocean-500 font-display text-xs font-bold text-white sm:left-1/2">
            ...
            {team.map((t, i) => (
              ...
              >
                <div className="relative aspect-[3/4] overflow-hidden rounded-2xl shadow-sm">
                  <Image src={t.photo} alt={t.name} fill sizes="(max-width: 640px) 50vw, 25vw" className="object-cover transition-transform duration-500 group-hover:scale-105" />
                  <div className="absolute inset-0 flex items-end justify-center gap-2 bg-deep-950/0 pb-3 opacity-0 transition-all duration-300 group-hover:bg-deep-950/30 group-hover:opacity-100">
                    <span className="grid h-8 w-8 place-items-center rounded-full bg-white/90 text-ocean-700">
      ...
      <AnimatePresence>
        {active && (
          ...
          >
            <button onClick={() => setActive(null)} className="absolute right-5 top-5 grid h-10 w-10 place-items-center rounded-full bg-white/10 text-white hover:bg-white/20" aria-label="Đóng">
              <X className="h-5 w-5" />
            </button>
            <motion.div initial={{ scale: 0.95 }} animate={{ scale: 1 }} onClick={(e) => e.stopPropagation()} className="max-w-2xl">
              <img src={active.src} alt={active.caption} className="max-h-[75dvh] w-full rounded-2xl object-contain" />
              <p className="mt-4 text-center text-sm text-white/80">{active.caption}</p>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* Trích dẫn + CTA */}
      <section className="relative overflow-hidden bg-horizon py-16 sm:py-20">
        <div className="relative mx-auto max-w-2xl px-5 text-center sm:px-8">
          <SectionReveal>
            <Quote className="mx-auto h-9 w-9 text-deep-900/40" />
